...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	...
	for _, entry := range workout.Entries {
		query := `
		INSERT INTO workout_entries(workout_id,exercise_name,sets,reps,duration_seconds,notes,weight,order_index)
		VALUES($1,$2,$3,$4,$5,$6,$7,$8)
		`
		_, err := tx.Exec(query, workout.ID, entry.ExerciesName, entry.Sets, entry.Reps, entry.DurationSeconds, entry.Notes, entry.Weight, entry.OrderIndex)
		if err != nil {
			return err
		}
	}
...
func (pg *PostgresWorkoutStore) GetWorkoutOwner(id int) (int, error) {
	var UserID int
	query := `
	SELECT user_id
	FROM workouts
